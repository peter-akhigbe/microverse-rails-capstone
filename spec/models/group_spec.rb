...
RSpec.describe Group, type: :model do
  ...
  it 'is valid with valid attributes' do
    group = Group.new(name: 'Example Group', icon: 'example_icon', user:)
    expect(group).to be_valid
  end

  it 'is not valid without a name' do
    group = Group.new(icon: 'example_icon', user:)
  ...
  it 'destroys associated group_entities when destroyed' do
    ...
    GroupEntity.create(group_id: group.id, entity_id: entity.id)
